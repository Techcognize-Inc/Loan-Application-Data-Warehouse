...
def write_jdbc(df: DataFrame, table_name: str, mode: str = "overwrite"):
    (
        ...
        .option("batchsize", "1000")         # smaller batch = less memory pressure
        .option("numPartitions", "1")        # single writer
        .option("isolationLevel", "NONE")
        .option("stringtype", "unspecified") # safer for mixed types (optional)
        .jdbc(
            url=jdbc_url(),
            table=table_name,
            properties=jdbc_properties(),
...
# ---------------------------------------------------------------------------
# Partitioned JDBC read helper (prevents OOM / executor crashes on big tables)
# ---------------------------------------------------------------------------
def read_jdbc_partitioned(
    spark: SparkSession,
    table: str,
    partition_col: str,
    num_partitions: int = 8
) -> DataFrame:
    bounds_q = f'(SELECT MIN("{partition_col}") AS lo, MAX("{partition_col}") AS hi FROM {table}) t'
    row = spark.read.jdbc(url=jdbc_url(), table=bounds_q, properties=jdbc_properties()).collect()[0]
    lo, hi = row["lo"], row["hi"]

    # fallback if table empty or no range
    if lo is None or hi is None or lo == hi:
        return read_jdbc(spark, table)
...
def validate_application_train(df: DataFrame):
    ...
    reject_reason_expr = (
        ...
        .when(col("CODE_GENDER").isNotNull() & (~col("CODE_GENDER").isin("M", "F", "XNA")), lit("INVALID_CODE_GENDER"))
    )
...
def validate_bureau(df: DataFrame):
    ...
    reject_reason_expr = (
        ...
        .when(col("CREDIT_DAY_OVERDUE").isNotNull() & col("CREDIT_DAY_OVERDUE_DBL").isNull(),   lit("INVALID_CREDIT_DAY_OVERDUE_TYPE"))
        .when(col("CREDIT_DAY_OVERDUE_DBL").isNotNull() & (col("CREDIT_DAY_OVERDUE_DBL") < 0), lit("NEGATIVE_CREDIT_DAY_OVERDUE"))
    )
...
def main():
    ...
    for table_name in TABLES:
        ...
        # Partitioned reads for big tables (prevents executor crash)
        if table_name == "bureau_balance":
            df = read_jdbc_partitioned(spark, raw_table, partition_col="SK_ID_BUREAU", num_partitions=8)
        elif table_name in ["pos_cash_balance", "installments_payments", "credit_card_balance"]:
            df = read_jdbc_partitioned(spark, raw_table, partition_col="SK_ID_CURR", num_partitions=8)
        else:
            df = read_jdbc(spark, raw_table)

        validator = VALIDATORS[table_name]
        valid_df, rejects_df = validator(df)

        # Single writer to Postgres
        valid_df = valid_df.coalesce(1)
        rejects_df = rejects_df.coalesce(1)

        print(f"  Writing valid rows → {silver_table}")
        write_jdbc(valid_df, silver_table, mode="overwrite")

        print(f"  Writing rejects → {quarantine_table}")
        write_jdbc(rejects_df, quarantine_table, mode="overwrite")
